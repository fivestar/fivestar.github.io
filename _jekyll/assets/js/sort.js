...
document.addEventListener('DOMContentLoaded', function () {
  new _sortable2.default(document.querySelector('.js-sortable'));
});

},{"./sortable":299,"babel-polyfill":1}],299:[function(require,module,exports){
'use strict';
...
var Sortable = function () {
  function Sortable(containerElement) {
    ...
    this.config = Object.assign({}, {
      ghostClass: 'ghost',
      itemAttribute: 'data-sortable-item'
    }, config);
  ...
  _createClass(Sortable, [{
    ...
    value: function initEvents() {
      ...
      try {
        for (var _iterator = Array.from(this.containerElement.children)[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          var itemElement = _step.value;

          var handleElement = itemElement.querySelector('.js-sortable-handle');

          handleElement.itemElement = itemElement;
          itemElement.setAttribute(this.config.itemAttribute, '');

          handleElement.addEventListener('mousedown', this.dispatchStart.bind(this));
          handleElement.addEventListener('mouseup', this.dispatchEnd.bind(this));
          handleElement.addEventListener('touchstart', this.dispatchStart.bind(this));
          handleElement.addEventListener('touchend', this.dispatchEnd.bind(this));
          handleElement.addEventListener('touchcancel', this.dispatchEnd.bind(this));
          handleElement.addEventListener('touchmove', this.emulateDragOver.bind(this));

          itemElement.addEventListener('dragstart', this.handleDragStart.bind(this));
          itemElement.addEventListener('dragend', this.handleDrop.bind(this));
    ...
    key: 'dispatchStart',
    value: function dispatchStart(event) {
      var itemElement = (event.touches && event.touches[0] || event).target.itemElement;
      var dragEvent = new Event('dragstart');

      itemElement.draggable = true;
      itemElement.dispatchEvent(dragEvent);
    }
  }, {
    key: 'dispatchEnd',
    value: function dispatchEnd(event) {
      var itemElement = event.target.itemElement;
    ...
    value: function handleDrop(event) {
      ...
      this.dragElement.classList.remove(this.config.ghostClass);
      this.dragElement.removeAttribute('draggable');
    ...
    value: function handleDragOver(event) {
      ...
      try {
        this.switchItem(this.dragElement, event.target);
      } catch (e) {
        console.error(e);
      }
    }
    ...
    key: 'switchItem',
    value: function switchItem(sortElement, targetElement) {
      while (!targetElement.hasAttribute(this.config.itemAttribute)) {
        if (!targetElement.parentElement) {
          throw new Error('Unexpected drag over');
        }

        targetElement = targetElement.parentElement;
      }

      if (sortElement === targetElement) {
        return;
      }

      var itemArray = Array.from(this.containerElement.children);
      var sortIndex = itemArray.indexOf(sortElement);
      var targetIndex = itemArray.indexOf(targetElement);

      if (sortIndex < 0 || targetIndex < 0) {
        throw new Error('Invalid sort target: ' + sortIndex + ' <-> ' + targetIndex);
      }

      this.containerElement.insertBefore(sortElement, sortIndex > targetIndex ? targetElement : targetElement.nextSibling);
    }
  }]);
